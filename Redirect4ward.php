...
class Redirect4ward extends Controller
{
	...
	function getPageIdFromURL($urlfragments)
	{
		...
		$objTarget = $this->Database->prepare('SELECT jumpTo,type,jumpToType,externalUrl FROM tl_redirect4ward WHERE published=? AND url=? AND (host=? OR CONCAT("www.",host)=? OR host="")')
